...
fn get_include_path(dkp_path: String) -> Vec<String> {
    ...
    let dkp_gcc = format!("{}/devkitPPC/bin/powerpc-eabi-gcc", dkp_path);
    let gcc_output = match Command::new(dkp_gcc)
        .arg("-xc")
        .arg("-E")
        .arg("-v")
        .arg("/dev/null")
        .output()
    {
        Ok(output) => output,
        Err(e) => panic!(
            "failed to get the default include paths on the host machine!\n{}",
            e
        ),
    };
    ...
    parsed_output
        .split("\n")
        .filter(|line| line.trim().starts_with(&dkp_path) && line.contains("include"))
        .for_each(|line| {
