...
impl Gx {
    ...
    pub fn set_copy_filter(
        aa: u8,
        mut sample_pattern: [[u8; 2]; 12],
        vf: u8,
        mut v_filter: [u8; 7],
    ) {
        unsafe { extra_sys::GX_SetCopyFilter(aa, &mut sample_pattern, vf, &mut v_filter) }
    }
    ...
    pub fn load_projection_mtx(mut mt: Mtx44, p_type: u8) {
        unsafe { extra_sys::GX_LoadProjectionMtx(&mut mt, p_type) }
    }
    ...
    pub fn load_pos_mtx_imm(mut mt: Mtx34, pnidx: u32) {
        unsafe { extra_sys::GX_LoadPosMtxImm(&mut mt, pnidx) }
    }
...
mod extra_sys {
    use super::{Mtx34, Mtx44};

    extern "C" {
        pub fn GX_LoadProjectionMtx(mt: *mut Mtx44, type_: u8);
        pub fn GX_LoadPosMtxImm(mt: *mut Mtx34, pnidx: u32);
        pub fn GX_SetCopyFilter(
            aa: u8,
            sample_pattern: *mut [[u8; 2usize]; 12],
            vf: u8,
            vfilter: *mut [u8; 7],
        );
    }
