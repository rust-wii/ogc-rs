...
//!
//! This module implements a safe wrapper around the networking functions found
//! in ``network.h``.
...
/// Implementation to convert from a ``IPV4Address`` into a ``in_addr`` used by
/// ogc_sys.
...
/// This function converts the specified string in the Internet standard dot
...
/// This function call converts the specified string in the Internet standard
/// dot notation to a network address, and stores the address in the structure
/// provided. The converted address will be in Network Byte Order.
...
/// This function returns a structure of type ``HostInformation`` for the given
/// host name. Here ``addr_string`` is either a hostname, or an IPv4 address in
...
impl Network {
	...
	pub fn new_socket(
		...
		let r =
			unsafe { ffi::net_socket(domain.into(), socket_type.into(), 0) };
...
impl Socket {
	...
	pub fn connect(
		...
		let r = unsafe {
			ffi::net_connect(self.0, socket_addr.into(), address_length)
		};
	...
	pub fn bind(
		...
		let r = unsafe {
			ffi::net_bind(self.0, socket_addr.into(), address_length)
		};
	...
	/// This function is called only by a TCP server to listen for the client
	/// request.
	...
	/// The accept function is called by a TCP server to accept client requests
	/// and to establish actual connection.
	pub fn accept(
		&self,
		socket_addr: SocketAddress,
		...
		let r = unsafe {
			ffi::net_accept(self.0, socket_addr.into(), address_length)
		};
	...
	pub fn write(descriptor: i32, buffer: &[u8], count: i32) -> Result<i32> {
		let r = unsafe {
			ffi::net_write(descriptor, buffer.as_ptr() as *const c_void, count)
		};
	...
	pub fn read(descriptor: i32, buffer: &mut [u8], count: i32) -> Result<i32> {
		let r = unsafe {
			ffi::net_read(descriptor, buffer.as_ptr() as *mut c_void, count)
		};
