...
impl Thread {
	...
	/// Suspend this thread.
	///
	/// On success, returns `Ok(1)` if the thread was already suspended, or
	/// `Ok(0)` if it was successfully suspended. Otherwise, returns an error
	/// code.
	...
	/// Resume this thread.
	///
	/// On success, returns `Ok(1)` if the thread was already resumed, or
	/// `Ok(0)` if it was successfully resumed. Otherwise, returns an error
	/// code.
...
impl Queue {
	...
	/// Removes all blocked threads from the thread synchronization queue and
	/// sets them back to running state.
	pub fn broadcast(&self) {
		unsafe { ffi::LWP_ThreadBroadcast(self.handle) }
	}

	/// Signals one thread to be revmoved from the thread synchronization queue
	/// and sets it back to running state.
	pub fn signal(&self) {
...
/// Yield the current thread to another one with higher priority or, if not
/// running, at the same priority whose state is runnable.
pub fn yield_now() {
...
/// Pushes the current thread onto the given thread synchronization queue and
/// sets the thread state to blocked.
