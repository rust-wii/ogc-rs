...
//!
//! This module implements a safe wrapper around the audio functions found in
//! ``asndlib.h``.
...
impl VoiceOptions {
	...
	/// Voice slot to use for this sound. Valid values are `0..16`
	/// non-inclusive.
	#[must_use]
...
impl Asnd {
	...
	/// De-initializes the asnd lib. This is also called when `Asnd` gets
	/// dropped.
	pub fn end() {
		unsafe {
			ffi::ASND_End();
	...
	/// Returns the global sample counter. Can be used to implement timers with
	/// high precision.
	...
	/// `Asnd::status_voice()` to test status. The voices are played in 16-bit
	/// stereo, regardless of source format. The buffer MUST be aligned and
	/// padded to 32 bytes.
	pub fn set_voice(
	...
	/// Sets a PCM voice to play infinitely. See `Asnd::set_voice()` as it is
	/// largely identical. The buffer MUST be aligned and padded to 32 bytes.
	pub fn set_infinite_voice(
		options: VoiceOptions,
		sound_buffer: &mut [u8],
	) -> Result<()> {
		Self::validate_buffer(sound_buffer);
	...
	/// Adds a PCM voice to play from the second buffer. Sound buffer must be
	/// 32-byte aligned and have same sample format as first buffer. This must
	/// only be called after `Asnd::set_voice()`, which must return `Ok()`.
	...
	pub fn change_pitch_voice(voice: u32, pitch: u32) -> Result<()> {
		assert!(voice < 16, "Voice index {} is >= 16", voice);
		let err =
			unsafe { ffi::ASND_ChangePitchVoice(voice as i32, pitch as i32) };
		if_not!(SND_OK => "Asnd::change_pitch_voice() failed with error {}", err)
	}

	/// Changes the voice volume in real time. This function can be used to
	/// create audio effects like distance attenuation.
	pub fn change_volume_voice(
		voice: u32,
		volume_left: u8,
		volume_right: u8,
	) -> Result<()> {
	...
	/// Returns the voice tick counter. This value represents the number of
	/// ticks since this voice started to play, sans delay time. If the lib is
	/// initialized with `INIT_RATE=48000`, a return value of 24000 is equal to
	/// 0.5 seconds.
	...
	/// Returns the voice playback time. This value represents the time in
	/// milliseconds since this voice started playing.
	...
	/// This must be the same pointer sent to `Asnd::add_voice()` or
	/// `Asnd::set_voice()`. Returns 0 if the pointer is unused.
