...
//!
//! This module implements a safe wrapper around the graphics functions found in
//! ``gx.h``.
...
pub const GX_PIPE: VolAddress<u8, (), Safe> =
	unsafe { VolAddress::new(0xCC00_8000) };
...
///
/// Which type you use depends on your needs; however, performance can increase
/// by using triangle strips or fans instead of discrete triangles.
#[derive(Copy, Clone, Debug)]
#[repr(u8)]
pub enum Primitive {
	/// Draws a series of unconnected quads. Every four vertices completes a
	/// quad. Internally, each quad is translated into a pair of triangles.
	Quads = ffi::GX_QUADS as _,

	/// Draws a series of unconnected triangles. Three vertices make a single
	/// triangle.
	Triangles = ffi::GX_TRIANGLES as _,

	/// Draws a series of triangles. Each triangle (besides the first) shares a
	/// side with the previous triangle. Each vertex (besides the first two)
	/// completes a triangle.
	TriangleStrip = ffi::GX_TRIANGLESTRIP as _,

	/// Draws a single triangle fan. The first vertex is the "centerpoint". The
	/// second and third vertex complete the first triangle. Each subsequent
	/// vertex completes another triangle which shares a side with the previous
	/// triangle (except the first triangle) and has the centerpoint vertex as
	/// one of the vertices.
	TriangleFan = ffi::GX_TRIANGLEFAN as _,

	/// Draws a series of unconnected line segments. Each pair of vertices
	/// makes a line.
	Lines = ffi::GX_LINES as _,

	/// Draws a series of lines. Each vertex (besides the first) makes a line
...
/// Destination (`dst`) acquires the value of one of these operations, given in
/// Rust syntax.
#[derive(Copy, Clone, Debug)]
#[repr(u8)]
pub enum LogicOp {
...
/// Performance counter 0 is used to measure attributes dealing with geometry
/// and primitives, such as triangle counts and clipping ratios.
///
/// `Perf0::Xf*` measure how many GP cycles are spent in each stage of the XF.
///
/// The triangle metrics (`Perf0::Triangles*`) allow counting triangles under
/// specific conditions or with specific attributes.
///
/// `Perf0::Triangles*Tex` count triangles based on the number of texture
/// coordinates supplied; `Perf0::Triangles*Clr` count triangles based on the
/// number of colors supplied.
///
/// The quad metrics allow you to count the number of quads (2x2 pixels) the GP
/// processes. The term coverage is used to indicate how many pixels in the quad
/// are actually part of the triangle being rasterized. For example, a coverage
/// of 4 means all pixels in the quad intersect the triangle. A coverage of 1
/// indicates that only 1 pixel in the quad intersected the triangle.
#[derive(Copy, Clone, Debug)]
#[repr(u32)]
pub enum Perf0 {
	...
	/// Number of GP clocks that have elapsed since the previous call to
	/// `Gx::read_gp0_metric()`.
	...
	/// Number of triangles that *failed* the front-face/back-face culling
	/// test.
	TrianglesCulled = ffi::GX_PERF0_TRIANGLES_CULLED,
	/// Number of triangles that *passed* the front-face/back-face culling
	/// test.
	...
	/// Number of cycles the XF is waiting on input. If the XF is waiting a
	/// large percentage of the total time, it may indicate that the CPU is not
	/// supplying data fast enough to keep the GP busy.
	XfWaitIn = ffi::GX_PERF0_XF_WAIT_IN,
	/// Number of cycles the XF waits to send its output to the rest of the GP
	/// pipeline. If the XF cannot output, it may indicate that the GP is
	/// currently fill-rate limited.
	XfWaitOut = ffi::GX_PERF0_XF_WAIT_OUT,
	/// Number of cycles the transform engine is busy.
	XfXfrmClks = ffi::GX_PERF0_XF_XFRM_CLKS,
}

/// Performance counter 1 is used for measuring texturing and caching
/// performance as well as FIFO performance.
///
/// `Perf1::Tc*` can be used to compute the texture cache (TC) miss rate. The
/// `TcCheck*` parameters count how many texture cache lines are accessed for
/// each pixel. In the worst case, for a mipmap, up to 8 cache lines may be
/// accessed to produce one textured pixel. `Perf1::TcMiss` counts how many of
/// those accesses missed the texture cache. To compute the miss rate, divide
...
pub enum Perf1 {
	...
	/// Number of GP clocks that have elapsed since the last call to
	/// `Gx::read_gp1_metric()`.
	Clocks = ffi::GX_PERF1_CLOCKS,
	/// Counts all requests (32B/request) from the GP Command Processor (CP).
	/// It should be equal to the sum of counts returned by `Perf1::FifoReq`,
	/// `Perf1::CallReq`, and `Perf1::VcMissReq`.
	...
	/// Number vertex cache miss request. Each miss requests a 32B transfer
	/// from main memory.
...
/// Specifies whether the input source color for a color channel comes from a
/// register or a vertex.
#[derive(Copy, Clone, Debug)]
#[repr(u8)]
pub enum Source {
...
///
/// The Graphics FIFO is the mechanism used to communicate graphics commands
/// from the CPU to the Graphics Processor (GP). The FIFO base pointer is
/// 32-byte aligned and the size must be a multiple of 32 bytes.
///
/// The CPU's write-gather pipe is used to write data to the FIFO. Therefore,
/// the FIFO memory area must be forced out of the CPU cache prior to being
/// used. `DCInvalidateRange()` may be used for this purpose. Due to the
/// mechanics of flushing the write-gather pipe, the FIFO memory area should be
/// at least 32 bytes larger than the maximum expected amount of data stored. Up
/// to 32 NOPs may be written at the end during flushing.
#[derive(Debug)]
#[repr(transparent)]
...
impl Fifo {
	...
	///
	/// The high and low water marks are used in *immediate-mode*, i.e. when the
	/// fifo is attached to both the CPU and Graphics Processor (GP) (see
	/// `Gx::set_cpu_fifo()` and `Gx::set_gp_fifo()`).
	///
	/// The hardware keeps track of the number of bytes between the read and
	/// write pointers. This number represents how full the FIFO is, and when it
	/// is greater than or equal to the *hiwatermark*, the hardware issues an
	/// interrupt. The GX API will suspend sending graphics to the Graphics FIFO
	/// until it has emptied to a certain point. The *lowatermark* is used to
	/// set the point at which the FIFO is empty enough to resume sending
	/// graphics commands to the FIFO. Both *hiwatermark* and *lowatermark*
	/// should be in multiples of 32B. The count for *lowatermark* should be
	/// less than *hiwatermark*. Of course, *hiwatermark* and *lowatermark* must
	/// be less than the size of the FIFO.
	///
	/// # Note
	/// When the FIFO is only attached to the CPU or only attached to the GP,
	/// the high and low watermark interrupts are disabled.
	...
	///
	/// # Note
	/// The count is incorrect if an overflow has occurred (i.e. you have
	/// written more data than the size of the fifo), as the hardware cannot
	/// detect an overflow in general.
	...
	/// Returns a non-zero value if the write pointer has passed the TOP of the
	/// FIFO.
	///
	/// Returns true only if the FIFO is attached to the CPU and the FIFO write
	/// pointer has passed the top of the FIFO. Use the return value to detect
	...
	///
	/// # Note
	/// If the FIFO write pointer is not explicitly set to the base of the FIFO,
	/// you cannot rely on this function to detect overflows.
	...
	///
	/// # Note
	/// This is normally done only during initialization of the FIFO. After
...
impl Light {
	/// Creates a white spotlight with the given normal at the view-space
	/// origin, and with angular and distance attenuation turned off.
	///
	/// If needed, these are the default angle (*a*) and distance (*k*)
	/// coefficients:
	///
	...
	pub fn new_spotlight(nx: f32, ny: f32, nz: f32) -> Self {
		...
		// SAFETY: According to libogc source, light structs have 5 parts that
		// must be initialized:
		// + position: set to view-space origin with GX_InitLightPos()
		// + color: set to white with GX_InitLightColor().
		// + direction/half-angle vector: set to the given values with
		//   GX_InitLightDir().
	...
	/// Creates a white specular light with the given normal, and with angular
	...
	pub fn new_specular(nx: f32, ny: f32, nz: f32) -> Self {
		...
		// + direction/half-angle vector: set to the given values by
		//   GX_InitSpecularDir().
	...
	/// Sets coefficients used in the lighting attenuation calculation in a
	/// given light object.
	///
	/// The parameters `a0`, `a1`, and `a2` are used for angular (spotlight)
	/// attenuation. The coefficients `k0`, `k1`, and `k2` are used for distance
	/// attenuation. The attenuation function is:
	///
	/// `clamp0(a2^2 * aattn^2 + a1 * aattn + a0) / (k2 * d^2 + k1 * d + k0)`
	///
	/// where `aattn` is the cosine of the angle between the light direction and
	/// the vector from the light position to the vertex, and `d` is the
	/// distance from the light position to the vertex when the channel
	/// attenuation function is `AttnFn::Spot`. The light color will be
	/// multiplied by the `atten` factor when the attenuation function for the
	/// color channel referencing this light is set to `AttnFn::Spot` (see
	/// [`Gx::set_channel_controls()`]).
	///
	/// # Note
	/// The convenience function [`Light::spot_attn()`] can be used to set the
	/// angle attenuation coefficents based on several spot light types. The
	/// convenience function [`Light::dist_attn()`] can be used to set the
	/// distance attenuation coefficients using one of several common
	/// attenuation functions.
	///
	/// The convenience macro [`Light::shininess()`] can be used to set the
	/// attenuation parameters for specular lights.
	///
	/// When the channel attenuation function is set to `AttnFn::Spec`, the
	/// `aattn` and `d` parameter are equal to the dot product of the eye-space
	/// vertex normal and the half-angle vector set by
	/// [`Light::specular_dir()`].
	pub fn attn(
		&mut self,
		a0: f32,
	...
	///
	/// In reality, shininess is a property of the material being lit, not the
	/// light. However, in the Graphics Processor, the specular calculation is
	/// implemented by reusing the diffuse angle/distance attenuation function,
	/// so shininess is determined by the light attenuation parameters (see
	/// [`Light::attn()`]). Note that the equation is attempting to approximate
	/// the function `(N*H)^shininess`. Since the attenuation equation is only a
	/// ratio of quadratics, a true exponential function is not possible. To
	/// enable the specular calculation, you must set the attenuation parameter
	/// of the lighting channel to `AttnFn::Spec` using
	/// [`Gx::set_channel_controls()`].
	pub fn shininess(&mut self, shininess: f32) -> &mut Self {
		self.attn(0.0, 0.0, 1.0, shininess / 2.0, 0.0, 1.0 - shininess / 2.0)
	}

	/// Sets coefficients used in the lighting angle attenuation calculation in
	/// a given light object.
	///
	/// The parameters `a0`, `a1`, and `a2` are used for angular (spotlight)
	/// attenuation. The attenuation function is:
	///
	/// `clamp0(a2^2 * cos(x)^2 + a1 * cos(x) + a0) / (k2 * d^2 + k1 * d + k0)`
	///
	/// where `cos(x)` is the cosine of the angle between the light normal and
	/// the vector from the light position to the vertex, and `d` is the
	/// distance from the light position to the vertex. The `k0-2` coefficients
	/// can be set using [`Light::attn_k()`]. You can set both the `a0-2` and
	/// `k0-2` coefficients can be set using [`Light::attn()`]. The light color
	/// will be multiplied by the atten factor when the attenuation function for
	/// the color channel referencing this light is set to `AttnFn::Spot` (see
	/// [`Gx::set_channel_controls()`]).
	///
	/// # Note
	/// The convenience function [`Light::spot_attn()`] can be used to set the
	...
	/// Sets coefficients used in the lighting distance attenuation calculation
	/// in a given light object.
	///
	/// The coefficients `k0`, `k1`, and `k2` are used for distance attenuation.
	...
	/// where `cos(x)` is the cosine of the angle between the light normal and
	/// the vector from the light position to the vertex, and `d` is the
	/// distance from the light position to the vertex. The `a0-2` coefficients
	/// can be set using [`Light::attn_a()`]. You can set both the `a0-2` and
	/// `k0-2` coefficients can be set using [`Light::attn()`]. The light color
	/// will be multiplied by the atten factor when the attenuation function for
	/// the color channel referencing this light is set to `AttnFn::Spot` (see
	/// [`Gx::set_channel_controls()`]).
	...
	///
	/// This direction is used when the light object is used as spotlight or a
	/// specular light (see the `attn_fn` parameter of
	/// [`Gx::set_channel_controls()`]).
	///
	/// # Note
	/// The coordinate space of the light normal should be consistent with a
	/// vertex normal transformed by a normal matrix; i.e., it should be
	/// transformed to view space.
	///
	/// This function does not set the direction of parallel directional diffuse
	/// lights. If you want parallel diffuse lights, you may put the light
	/// position very far from every objects to be lit. (See [`Light::pos()`]
	/// and [`Gx::set_channel_controls()`])
	...
	///
	/// This function uses three easy-to-control parameters instead of `k0`,
	/// `k1`, and `k2` in [`Light::attn()`].
	///
	/// In this function, you can specify the brightness on an assumed reference
	/// point. The parameter `ref_dist` is distance between the light and the
	/// reference point. The parameter `ref_brite` specifies ratio of the
	/// brightness on the reference point. The value for `ref_dist` should be
	/// greater than 0 and that for ref_brite should be within
	/// `0 < ref_brite < 1`, otherwise distance attenuation feature is turned
	/// off. The parameter `dist_fn` defines type of the brightness decreasing
	/// curve by distance; `DistFn::Off` turns distance attenuation feature off.
	///
	/// # Note
	/// If you want more flexible control, it is better to use [`Light::attn()`]
	/// and calculate appropriate coefficients.
	///
	/// This function sets parameters only for distance attenuation. Parameters
	/// for angular attenuation should be set by using [`Light::spot_attn()`] or
	/// [`Light::attn_a()`].
	pub fn dist_attn(
		&mut self,
		ref_dist: f32,
	...
	///
	/// The GameCube graphics hardware supports local diffuse lights. The
	/// position of the light should be in the same space as a transformed
	/// vertex position (i.e., view space).
	///
	/// # Note
	/// Although the hardware doesn't support parallel directional diffuse
	/// lights, it is possible to get "almost parallel" lights by setting
	/// sufficient large values to position parameters (x, y and z) which makes
	/// the light position very far away from objects to be lit and all rays
	...
	///
	/// This function uses two easy-to-control parameters instead of `a0`, `a1`,
	/// and `a2` on [`Light::attn()`].
	///
	/// The parameter `cut_off` specifies cutoff angle of the spotlight by
	/// degree. The spotlight works while the angle between the ray for a vertex
	/// and the light direction given by [`Light::dir()`] is smaller than this
	/// cutoff angle. The value for `cut_off` should be within
	/// `0 < cut_off <= 90.0`, otherwise given light object doesn't become a
	/// spotlight.
	///
	/// The parameter `spotfn` defines type of the illumination distribution
	/// within cutoff angle. The value `SpotFn::Off` turns spotlight feature off
	/// even if color channel setting is using `AttnFn::Spot` (see
	/// [`Gx::set_channel_controls()`]).
	///
	/// # Note
	/// This function can generate only some kind of simple spotlights. If you
	/// want more flexible control, it is better to use [`Light::attn()`] and
	/// calculate appropriate coefficients.
	///
	/// This function sets parameters only for angular attenuation. Parameters
	/// for distance attenuation should be set by using [`Light::dist_attn()`]
	/// or [`Light::attn_k()`].
	...
	///
	/// This direction is used when the light object is used only as specular
	/// light. The coordinate space of the light normal should be consistent
	/// with a vertex normal transformed by a normal matrix; i.e., it should be
	/// transformed to view space.
	///
	/// # Note
	/// This function should be used if and only if the light object is used as
	/// specular light. One specifies a specular light in
	/// [`Gx::set_channel_controls()`] by setting the [attenuation function]
	/// (`AttnFn`) to `AttnFn::Spec`. Furthermore, one must not use
	/// [`Light::dir()`] or [`Light::pos()`] to set up a light object which will
	/// be used as a specular light since these functions will destroy the
	/// information set by [`Light::specular_dir()`]. In contrast to diffuse
	/// lights (including spotlights) that are considered local lights, a
	/// specular light is a parallel light (i.e. the specular light is
	/// infinitely far away such that all the rays of the light are parallel),
	/// and thus one can only specify directional information.
	...
	/// Sets the direction and half-angle vector of a specular light in the
	/// light object.
	///
	/// These vectors are used when the light object is used only as specular
	/// light. In contrast to [`Light::specular_dir()`], which caclulates
	/// half-angle vector automatically by assuming the view vector as
	/// (0, 0, 1), this function allows users to specify half-angle vector
	/// directly as input arguments. It is useful to do detailed control for
	/// orientation of highlights.
	///
...
impl Texture<'_> {
	/// Used to initialize or change a texture object for non-color index
	/// textures.
	pub fn new(
		img: &[u8],
		width: u16,
	...
	/// Used to initialize or change a texture object when the texture is colo
	/// index format.
	...
	///
	/// If set to `true`, the sum of LOD and `lodbias` (given in
	/// [`Texture::set_lod_bias()`]) is clamped so that it is never less than
	/// the minimum extent of the pixel projected in texture space. This
	/// prevents over-biasing the LOD when the polygon is perpendicular to the
	/// view direction.
	...
	///
	/// When set to `true`, the LOD is computed using adjacent texels; when
	/// `false`, diagonal texels are used instead. This should be set to `true`
	/// if you use bias clamping (see [`Texture::set_bias_clamp()`]) or
	/// anisotropic filtering (`GX_ANISO_2` or `GX_ANISO_4` for
	...
	///
	/// When the ratio of texels for this texture to pixels is not 1:1, the
	/// filter type for `minfilt` or `magfilt` is used. `minfilt` is used when
	/// the texel/pixel ratio is >= 1.0. `magfilt` is used when the texel/pixel
	/// ratio is < 1.0; needs to be `Near` or `Linear`.
	...
	/// Sets texture Level Of Detail (LOD) controls explicitly for a texture
	/// object.
	///
	/// It is the application's responsibility to provide memory for a texture
	/// object. When initializing a texture object using [`Texture::new()`] or
	/// [`Texture::with_color_idx()`], this information is set to default values
	/// based on the mipmap flag. This function allows the programmer to
	/// override those defaults.
	///
	/// # Note
	/// This function should be called after [`Texture::new()`] or
	/// [`Texture::with_color_idx()`] for a particular texture object.
	///
	/// Setting `biasclamp` prevents over-biasing the LOD when the polygon is
	/// perpendicular to the view direction.
	///
	/// `edgelod` should be set if `biasclamp` is set or `maxaniso` is set to
	/// `GX_ANISO_2` or `GX_ANISO_4`.
	///
	/// Theoretically, there is no performance difference amongst various
	/// magnification/minification filter settings except `GX_LIN_MIP_LIN`
	/// filter with `GX_TF_RGBA8` texture format which takes twice as much as
	...
	/// performance differences by changing filter modes (especially
	/// minification filter) because cache-hit ratio changes according to which
	/// filter mode is being used.
	pub fn set_lod(
		&mut self,
	...
	/// Allows one to modify the TLUT that is associated with an existing
	/// texture object.
	pub fn set_tlut(&mut self, tlut_name: u32) {
		unsafe { ffi::GX_InitTexObjTlut(&mut self.0, tlut_name) }
	}

	/// Allows one to modify the texture coordinate wrap modes for an existing
	/// texture object.
	pub fn set_wrap_mode(&mut self, wrap_s: WrapMode, wrap_t: WrapMode) {
		unsafe {
			ffi::GX_InitTexObjWrapMode(&mut self.0, wrap_s as u8, wrap_t as u8)
		}
	}
...
///
/// An array of these structures can be used to describe all the attributes in a
/// vertex. The attribute `GX_VA_NULL` should be used to mark the end of the
/// array.
...
#[derive(Clone, Copy, Debug)]
pub struct GpStatus {
	/// `true` if high watermark has been passed, or in the case when the CPU
	/// and GP FIFOs are the same, then `true` if the current GX thread is
	/// suspended.
	pub overhi: bool,
...
impl Gx {
	...
	///
	/// This function sets the default state of the graphics processor and
	/// should be called before any other GX functions. This function sets up an
	/// immediate-mode method of communicating graphics commands from the CPU to
	...
	///
	/// # Note
	/// It is also possible to override the default immediate-mode style and
	/// instead buffer the graphics for frame n+1 while the GP is reading the
	/// graphics for frame n. See [`Gx::set_cpu_fifo()`] and
	/// [`Gx::set_gp_fifo()`] for further information.
	///
	/// This function also designates the calling thread as the default GX
	/// thread; i.e., it assumes the calling thread is the one responsible for
	/// generating graphics data. This thread will be the thread to be suspended
	/// when the FIFO gets too full. The current GX thread can be changed by
	/// calling [`Gx::set_current_gx_thread()`].
	...
	///
	/// # Note
	/// If the FIFO is also attached to the CPU, the system is in immediate-
	/// mode, and the fifo acts like a true FIFO. In immediate-mode, graphics
	/// commands are fed directly from the CPU to the GP, and the FIFO's high
	/// and low water marks are active. The high and low water marks implement
	/// the flow-control mechanism between the CPU and GP. When the FIFO becomes
	/// more full than the high water mark, the CPU will stop writing graphics
	/// commands into the FIFO. When the FIFO empties to a point lower than the
	/// low water mark, the CPU will resume writing graphics commands into the
	/// FIFO. The high and low water marks are set with [`Fifo::set_limits()`].
	///
	/// If the FIFO is only attached to the GP, the FIFO acts like a buffer. In
	/// this case, high and low water marks are disabled, and the GP reads the
	/// FIFO until it is empty. Before attaching a new FIFO to the GP, you
	/// should make sure the previous FIFO is empty, using the *cmdIdle* status
	/// returned by [`Gx::get_gp_status()`].
	///
	/// The break point mechanism can be used to force the FIFO to stop reading
	/// commands at a certain point; see [`Gx::enable_breakpt()`].
	...
	///
	/// # Note
	/// If the FIFO being attached is one already attached to the GP, the FIFO
	...
	///
	/// This command will reset the entire graphics pipeline, including any
	/// commands in the graphics FIFO.
	///
	/// # Note
	/// Texture memory will not be reset, so currently loaded textures will
	/// still be valid; however, when loading texture using
	/// [`Gx::preload_entire_texture()`] or TLUTs using [`Gx::load_tlut()`], you
	/// must make sure the command completed. You can use the draw sync
	/// mechanism to do this; see [`Gx::set_draw_sync()`] and
	/// [`Gx::get_draw_sync()`].
	pub fn abort_frame() {
	...
	///
	/// The graphics hardware keeps track of a bounding box of pixel coordinates
	/// that are drawn in the Embedded Frame Buffer (EFB).
	...
	///
	/// # Note
	/// The counter's function is set using [`Gx::set_gp_metric()`]; the
	/// counter's value is read using [`Gx::read_gp_metric()`]. Consult these
	/// for more details.
	///
	/// # Safety
	/// This function resets CPU accessible counters, so it should **not** be
	...
	///
	/// This function clears the performance counter by sending a special clear
	/// token via the Graphics FIFO.
	///
	/// # Note
	/// To set the metric for the counter, call [`Gx::set_vcache_metric()`]; to
	...
	///
	/// # Note
	/// The break point feature allows the application to have two frames of
	/// graphics in the FIFO at the same time, overlapping one frame's
	/// processing by the graphics processor with another frame's processing by
	/// the CPU. For example, suppose you finish writing the graphics
	/// commands for one frame and are ready to start on the next. First,
	/// execute a [`Gx::flush()`] command to make sure all the data in the CPU
	/// write gatherer is flushed into the FIFO. This will also align the FIFO
	/// write pointer to a 32B boundary. Next, read the value of the
	/// current write pointer using [`Fifo::pointers()`]. Write the value of the
	/// write pointer as the break point address using `Gx::enable_breakpt()`.
	/// When the FIFO read pointer reaches the break point address the hardware
	/// will disable reads from the FIFO. The status `brkpt`, returned by
	/// [`Gx::get_gp_status()`], can be polled to detect when the break point is
	/// reached. The application can then decide when to disable the break
	/// point, using [`Gx::disable_breakpt()`], which will allow the FIFO to
	/// resume reading graphics commands.
	///
	/// FIFO reads will stall when the GP FIFO read pointer is equal to the
	/// break point address `break_pt`. To re-enable reads of the GP FIFO, use
	/// [`Gx::disable_breakpt()`].
	///
	/// Use [`Gx::set_breakpt_callback()`] to set what function runs when the
	/// breakpoint is encountered.
	pub fn enable_breakpt(break_pt: *mut u8) {
		unsafe { ffi::GX_EnableBreakPt(break_pt as _) }
	}

	/// Registers `cb` as a function to be invoked when a break point is
	/// encountered.
	///
	/// Passing `None` means no function will run. The return value is a pointer
	/// to the previously registered callback, if any.
	///
	/// # Safety
	/// The callback will run with interrupts disabled, so it should terminate
	...
	/// Allows reads from the FIFO currently attached to the Graphics Processor
	/// (GP) to resume.
	///
	/// See [`Gx::enable_breakpt()`] for an explanation of the FIFO break point
	/// feature.
	///
	/// # Note
	/// The breakpoint applies to the FIFO currently attached to the Graphics
	/// Processor (GP) (see [`Gx::set_gp_fifo()`]).
	pub fn disable_breakpt() {
		unsafe { ffi::GX_DisableBreakPt() }
	}

	/// Initialize the transformation unit (XF) rasterizer unit (RAS) to take
	/// performance measurements.
	///
	/// # Safety
	/// This function should be avoided; use the GP performance metric functions
	/// instead.
	...
	///
	/// `overhi` and `underlow` will indicate whether or not the watermarks have
	/// been reached. If the CPU and GP FIFOs are the same, then `overhi` will
	/// indicate whether or not the current GX thread is suspended. The value of
	/// `brkpt` can be used to determine if a breakpoint is in progress (i.e. GP
	/// reads are suspended; they are resumed by a call to
	/// [`Gx::disable_breakpt()`]). A callback can also be used to notify your
	/// application that the break point has been reached. (see
	/// [`Gx::set_breakpt_callback()`])
	pub fn get_gp_status() -> GpStatus {
		let mut ret = (0, 0, 0, 0, 0);
		unsafe {
			ffi::GX_GetGPStatus(
				&mut ret.0, &mut ret.1, &mut ret.2, &mut ret.3, &mut ret.4,
			);
		}
		GpStatus {
			overhi: ret.0 != 0,
	...
	/// Loads a light object into a set of hardware registers associated with a
	/// Light ID.
	///
	/// This function copies the light object data into the graphics FIFO
	/// through the CPU write-gather buffer mechanism. This guarantees that the
	/// light object is coherent with the CPU cache.
	///
	/// # Note
	/// The light object must have been initialized first using the necessary
	/// `GX_InitLight*()` functions.
	///
	...
	/// `Gx::set_array(GX_VA_LIGHTARRAY, ...)` points. Then it loads the object
	/// into the hardware register associated with Light ID.
	///
	/// # Note
	/// Data flows directly from the array in DRAM to the GP; therefore, the
	/// light object data may not be coherent with the CPU's cache. The
	/// application is responsible for storing the light object data from the
	/// CPU cache (using `DCStoreRange()`) before calling
	/// `Gx::load_light_idx()`.
	...
	///
	/// This function inserts a synchronization command into the graphics FIFO.
	/// When the GPU sees this command it will allow the rest of the pipe to
	/// flush before continuing. This command is useful in certain situation
	/// such as after using [`Gx::copy_tex()`] and before a primitive
	/// that uses the copied texture.
	///
	/// # Note
	/// The command is actually implemented by writing the control register that
	/// determines the format of the embedded frame buffer (EFB). As a result,
	...
	///
	/// The CPU fifo that was attached at the time
	/// [`Gx::redirect_write_gather_pipe()`] was called will be re-attached. If
	/// there is data pending in the write gather pipe (e.g. if the amount
	/// of data written was not a multiple of 32 bytes), the data will be padded
	/// with zeroes and flushed out.
	///
	/// # Safety
	/// This function must be called between successive calls to
	/// [`Gx::redirect_write_gather_pipe()`].
	...
	///
	/// When all previous commands have been processed and the pipeline is
	/// empty, a *DrawDone* status bit will be set, and an interrupt will occur.
	/// You can receive notification of this event by installing a callback on
	/// the interrupt with [`Gx::set_draw_done_callback()`], or you can poll the
	/// status bit with [`Gx::wait_draw_done()`]. This function also flushes the
	/// write-gather FIFO in the CPU to make sure that all commands are sent to
	/// the graphics processor.
	///
	/// # Note
	/// This function is normally used in multibuffer mode (see
	/// [`Gx::set_cpu_fifo()`]). In immediate mode, the [`Gx::draw_done()`]
	/// command can be used, which both sends the command and stalls until the
	/// *DrawDone* status is true.
	pub fn set_draw_done() {
		unsafe { ffi::GX_SetDrawDone() }
	}

	/// Inserts a synchronization command into the graphics FIFO. When the
	/// Graphics Processor sees this command, it will allow the texture pipeline
	/// to flush before continuing.
	///
	/// This command is necessary when changing the usage of regions of texture
	/// memory from preloaded or TLUT to cached areas. It makes sure that the
	/// texture pipeline is finished with that area of the texture memory prior
	/// to changing its usage. This function should be called prior to drawing
	/// any primitives that uses the texture memory region in its new mode. It
	/// is not necessary to call this command when changing texture memory
	/// regions from cached to preloaded (or TLUT), since the commands to load
	/// the regions with data will cause the necessary synchronization to happen
	/// automatically.
	...
	///
	/// It means all graphics commands sent before this *DrawDone* command have
	/// executed and the last pixel has been written to the frame buffer. You
	/// may want to execute some non-graphics operations between executing
	/// [`Gx::set_draw_done()`] and this function, but if you simply
	/// want to wait and have nothing to execute, you can use
	/// [`Gx::draw_done()`].
	///
	/// # Note
	/// This function is normally used in immediate mode (see
	/// [`Gx::set_cpu_fifo()`]). In multibuffer mode, sending the 'done' command
	/// is separated from polling the 'done' status (see [`Gx::set_draw_done()`]
	/// and [`Gx::wait_draw_done()`]).
	...
	///
	/// Normally, Z buffering should happen before texturing, as this enables
	/// better performance by not texturing pixels that are not visible;
	/// however, when alpha compare is used, Z buffering must be done after
	/// texturing (see [`Gx::set_alpha_compare()`]).
	...
	///
	/// The screen origin (`xOrig` = 0.0, `yOrig` = 0.0) is at the top left
	/// corner of the display. Floating point arguments allow the viewport to be
	/// adjusted by 1/2 line for interlaced field rendering modes; see
	/// [`GX_SetViewportJitter()`]. The viewport depth parameters are normalized
	/// coordinates from 0.0 - 1.0. The GX API will convert the depth range
	/// values to proper scale values depending on the type and format of the
	/// Z-buffer.
	///
	/// **Note**: You should avoid using negative values for `xOrig` or `yOrig`.
	/// While this may work, it may cause problems with points and lines being
	/// clipped incorrectly. If you need to shift the viewport up or left,
	/// consider using [`GX_SetScissorBoxOffset()`] instead.
	pub fn set_viewport(
		x_orig: f32,
		y_orig: f32,
		wd: f32,
	...
	/// Calculates an appropriate Y scale factor value for
	/// GX_SetDispCopyYScale() based on the height of the EFB and the height of
	/// the XFB. See [GX_GetYScaleFactor](https://libogc.devkitpro.org/gx_8h.html#a1558cf7d2eb9a6690fee4b64c4fc5a8e) for more.
	...
	/// Sets the subpixel sample patterns and vertical filter coefficients used
	/// to filter subpixels into pixels. See [GX_SetCopyFilter](https://libogc.devkitpro.org/gx_8h.html#afd65b7e5f2040ddb3352649efde72faf) for more.
	...
	/// Controls various rasterization and texturing parameters that relate to
	/// field-mode and double-strike rendering. See [GX_SetFieldMode](https://libogc.devkitpro.org/gx_8h.html#a13f0df0011d04c3d986135e800fbcd21) for more.
	pub fn set_field_mode(field_mode: bool, half_aspect_ratio: bool) {
		unsafe {
			ffi::GX_SetFieldMode(field_mode as u8, half_aspect_ratio as u8)
		}
	}
	...
	/// Enables or disables culling of geometry based on its orientation to the
	/// viewer.
	///
	/// Primitives in which the vertex order is clockwise to the viewer are
	/// considered front-facing.
	...
	/// Copies the embedded framebuffer (EFB) to the external framebuffer(XFB)
	/// in main memory.
	///
	/// # Safety
	///
	...
	/// Sets the gamma correction applied to pixels during EFB to XFB copy
	/// operation. See [GX_SetDispCopyGamma](https://libogc.devkitpro.org/gx_8h.html#aa8e5bc962cc786b2049345fa698d4efa) for more.
	pub fn set_disp_copy_gamma(gamma: Gamma) {
		unsafe { ffi::GX_SetDispCopyGamma(gamma.0) }
	}

	/// Sets the attribute format (vtxattr) for a single attribute in the Vertex
	/// Attribute Table (VAT). See [GX_SetVtxAttrFmt](https://libogc.devkitpro.org/gx_8h.html#a87437061debcc0457b6b6dc2eb021f23) for more.
	...
	/// Sets the number of texture coordinates that are generated and available
	/// for use in the Texture Environment TEV stages. See [GX_SetNumTexGens](https://libogc.devkitpro.org/gx_8h.html#a55a79a1688d3a6957ee0c37d6323d159) for more.
	pub fn set_num_tex_gens(nr: u32) {
		unsafe { ffi::GX_SetNumTexGens(nr) }
	}

	/// Simplified function to set various TEV parameters for this tevstage
	/// based on a predefined combiner mode. See [GX_SetTevOp](https://libogc.devkitpro.org/gx_8h.html#a68554713cdde7b45ae4d5ce156239cf8) for more.
	pub fn set_tev_op(tevstage: u8, mode: u8) {
		unsafe { ffi::GX_SetTevOp(tevstage, mode) }
	}

	/// Specifies the texture and rasterized color that will be available as
	/// inputs to this TEV tevstage. See [GX_SetTevOrder](https://libogc.devkitpro.org/gx_8h.html#ae64799e52298de39efc74bf989fc57f5) for more.
	...
	///
	/// # Note
	/// Preloaded textures (see [`Gx::preload_entire_texture()`]) are not
	/// affected.
	pub fn invalidate_tex_all() {
		unsafe { ffi::GX_InvalidateTexAll() }
	}

	/// Loads the state describing a texture into one of eight hardware register
	/// sets.
	///
	/// Before this happens, the texture object *obj* should be initialized
	/// using [`Texture::new()`] or [`Texture::with_color_idx()`]. The *mapid*
	/// parameter refers to the texture map slot that is set, and takes a value
	/// between 0 and 7 inclusive. Once loaded, the texture can be used in any
	/// Texture Environment (TEV) stage using [`Gx::set_tev_order()`].
	///
	/// # Note
	/// This function will call the functions set by
	/// [`Gx::set_tex_region_callback()`] (and
	/// [`Gx::set_tlut_region_callback()`] if the texture is color-index format)
	/// to obtain the texture regions associated with this texture object. These
	/// callbacks are set to default functions by [`Gx::init()`].
	///
	/// # Safety
	/// If the texture is a color-index texture, you **must** load the
	/// associated TLUT (using [`Gx::load_tlut()`]) before calling this
	/// function.
	...
	pub fn set_global_mat_color(color_channel: ColorChannel, color: Color) {
		match color_channel {
			ColorChannel::Color0 => {
				XFReg::MATERIAL0.load(u32::from_be_bytes([
					color.0.a, color.0.b, color.0.g, color.0.r,
				]))
			}
			ColorChannel::Color1 => {
				XFReg::MATERIAL1.load(u32::from_be_bytes([
					color.0.a, color.0.b, color.0.g, color.0.r,
				]))
			}
		}
	...
	///
	/// Specifically, this functions invalidates the vertex cache tags. This
	/// function should be used whenever you relocate or modify data that is
	/// read by, or may be cached by, the vertex cache. The invalidation is very
	/// fast, taking only two Graphics Processor (GP) clock cycles to complete.
	///
	/// # Note
	/// The vertex cache is used to cache indexed attribute data. Any attribute
	/// that is set to `GX_INDEX8` or `GX_INDEX16` in the current vertex
	/// descriptor (see [`Gx::set_vtx_desc()`]) is indexed. Direct data bypasses
	/// the vertex cache. Direct data is any attribute that is set
	/// to `GX_DIRECT` in the current vertex descriptor.
	pub fn inv_vtx_cache() {
		GX_PIPE.write(GPCommand::InvalidateVertexCache as u8);
	}

	/// Clears all vertex attributes of the current vertex descriptor to
	/// `GX_NONE`.
	///
	/// # Note
	/// The same functionality can be obtained using [`Gx::set_vtx_descv()`],
	/// however using [`Gx::clear_vtx_desc()`] is much more efficient.
	pub fn clear_vtx_desc() {
		unsafe { ffi::GX_ClearVtxDesc() }
	}

	/// Sets the type of a single attribute (attr) in the current vertex
	/// descriptor. See [GX_SetVtxDesc](https://libogc.devkitpro.org/gx_8h.html#af41b45011ae731ae5697b26b2bf97e2f) for more.
	pub fn set_vtx_desc(attr: VtxAttr, v_type: VtxDest) {
		unsafe { ffi::GX_SetVtxDesc(attr as u8, v_type.0) }
	}

	/// Used to load a 3x4 modelview matrix mt into matrix memory at location
	/// pnidx. See [GX_LoadPosMtxImm](https://libogc.devkitpro.org/gx_8h.html#a90349e713128a1fa4fd6048dcab7b5e7) for more.
	...
	pub fn load_tex_mtx_imm(mt: &mut Mtx34, pnidx: u32) {
		unsafe {
			ffi::GX_LoadTexMtxImm(mt as *mut _, pnidx, ffi::GX_MTX3x4 as _)
		}
	}
	...
	///
	/// # Note
	/// Only valid when the pixel format (see GX_SetPixelFmt()) is either
	/// `GX_PF_RGBA6_Z24` or `GX_PF_RGB565_Z16`.
	///
	/// Dithering should probably be turned off if you are planning on using the
	/// result of rendering for comparisons (e.g. outline rendering algorithm
	/// that writes IDs to the alpha channel, copies the alpha channel to a
	/// texture, and later compares the texture in the TEV).
	pub fn set_dither(dither: bool) {
		unsafe { ffi::GX_SetDither(dither as u8) }
	}

	/// Sends a DrawDone command to the GP and stalls until its subsequent
	/// execution.
	///
	/// # Note
	...
	pub fn set_z_mode(enable: bool, func: CmpFn, update_enable: bool) {
		unsafe {
			ffi::GX_SetZMode(enable as u8, func as u8, update_enable as u8)
		}
	}

	/// Determines how the source image, generated by the graphics processor, is
	/// blended with the Embedded Frame Buffer (EFB). See [GX_SetBlendMode](https://libogc.devkitpro.org/gx_8h.html#a1d9c43b161f3c5a30b9fd8ea182c8eb6) for more.
	...
	/// (EFB). See [GX_SetAlphaUpdate](https://libogc.devkitpro.org/gx_8h.html#ac238051bda896c8bb11802184882a2a0) for more.
	pub fn set_alpha_update(enable: bool) {
		unsafe { ffi::GX_SetAlphaUpdate(enable as u8) }
	}

	/// Enables or disables color-buffer updates when rendering into the
	/// Embedded Frame Buffer (EFB). See [GX_SetColorUpdate](https://libogc.devkitpro.org/gx_8h.html#a3978e3b08198e52d7cea411e90ece3e5) for more.
	...
	/// Sets the parameters for the alpha compare function which uses the alpha
	/// output from the last active TEV stage. See [Gx_SetAlphaCompare](https://libogc.devkitpro.org/gx_8h.html#a23ac269062a1b2c2efc8ad5aae24b26a) for more.
	pub fn set_alpha_compare(
		comp0: CmpFn,
		ref0: u8,
		aop: AlphaOp,
		comp1: CmpFn,
	...
	/// Sets the parameters for the alpha compare function which uses the alpha
	/// output from the last active TEV stage. See [GX_SetClipMode](https://libogc.devkitpro.org/gx_8h.html#a3d348d7af8ded25b57352e956f43d974) for more.
	...
	/// Allows the CPU to write color directly to the Embedded Frame Buffer
	/// (EFB) at position x, y. See [GX_PokeARGB](https://libogc.devkitpro.org/gx_8h.html#a5038d2f65e7959d64c68dcb1855353d8) for more.
