...
impl Audio {
	...
	pub fn init() -> Self {
		unsafe {
			...
			// libogc is fine with this, but this should be changed in the
			// future.
	...
	///
	/// Starts to transfer the data from main memory to the audio interface
	/// through DMA. This call should follow the call to ``init_dma`` which is
	/// used to initialize DMA transfers.
	fn start_dma() {
		unsafe {
			ffi::AUDIO_StartDMA();
		}
