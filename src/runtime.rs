...
/// Since the Wii has approximately 84MB of total memory, such an allocation is
/// probably an error.
...
/// In order for this to work ``Console`` and a minimal ``Video`` setup is
/// required!
#[panic_handler]
...
///
/// **Note**: The allocation error handler uses the ``println`` macro for
/// output. In order for this to work ``Console`` and a minimal ``Video`` setup
/// is required!
#[alloc_error_handler]
