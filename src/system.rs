...
//!
//! This module implements a safe wrapper around the OS functions found in
//! ``system.h``.
...
impl System {
	...
	/// Set the alarm parameters for a one-shot alarm, add to the list of alarms
	/// and start.
	pub fn set_alarm<F>(
		context: u32,
		fire_time: Duration,
		...
		F: Fn(u32, *mut c_void),
	{
		unsafe {
			...
			let code: extern "C" fn(alarm: u32, cb_arg: *mut c_void) =
				mem::transmute(ptr);
			let r = ffi::SYS_SetAlarm(
				context,
				timespec,
				Some(code),
				ptr::null_mut(),
	...
	/// Set the alarm parameters for a periodic alarm, add to the list of alarms
	/// and start. The alarm and interval persists as long as SYS_CancelAlarm()
	/// isn't called.
	...
	/// # Safety
	///
	/// The user must ensure this point into the memory is valid and the arena
	/// doesn't go out memory
	...
	pub fn set_reset_callback<F>(callback: Box<F>) {
		...
		unsafe {
			let code: extern "C" fn(irq: u32, ctx: *mut c_void) =
				mem::transmute(ptr);
